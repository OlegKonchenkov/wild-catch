...
function formatGameEvent(ev: PlayerGameEventRow): { icon: IconType; title: string; body: string } {
  ...
  switch (ev.type) {
    ...
    case 'trade_completed': {
      return { icon: GiShakingHands, title: 'Scambio completato!', body: 'Controlla il DaimonDex' }
    }
    case 'gold_forged': {
      return {
        icon: GiSparkles,
    ...
    case 'gym_taken': {
      ...
      return {
        icon: GiCrown,
        title: `Palestra conquistata${readStr(p, 'pin_name') ? `: ${readStr(p, 'pin_name')}` : '!'}`,
        body: dethroned ? 'Hai spodestato il titolare — ora presidi tu!' : 'Sei il primo a presidiarla!',
      }
    }
...
const EVENT_THEMES: Record<string, { color: string; dimColor: string; label: string }> = {
  ...
  trade_completed:   { color: '#60CDDD', dimColor: 'rgba(96,205,221,0.10)',  label: 'Scambio' },
