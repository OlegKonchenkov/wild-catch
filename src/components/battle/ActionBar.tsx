  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(180deg,transparent,rgba(3,5,9,.78) 38%,rgba(3,5,9,.95) 100%)',
      }}
    >
      {actions.map((a) => {
        ...
        return (
          <button
            ...
            style={{
              ...
              gap: a.primary ? 3 : 4, padding: a.primary ? '11px 12px' : '12px 6px',
              borderRadius: 18, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.55 : 1,
          ...
          >
            ...
            <span style={{ fontWeight: 800, fontSize: a.primary ? 15 : 12.5, letterSpacing: '.02em', textTransform: 'uppercase', lineHeight: 1 }}>
              {a.label}
            </span>
            {a.sub && (
              <span style={{ fontFamily: 'var(--font-mono, monospace)', fontSize: 10, fontWeight: 700, opacity: 0.92, marginTop: 1, padding: '1px 7px', borderRadius: 999, background: 'rgba(0,0,0,.22)' }}>{a.sub}</span>
            )}
          </button>
        )
      })}
