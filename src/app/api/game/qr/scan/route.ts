...
export async function POST(request: Request) {
  ...
  switch (qr.type) {
    ...
    case 'boss': {
      ...
      const { data: creaturesData } = await admin
        .from('creatures')
        .select('id, name, element, rarity, hp, atk, def, image_url, sprite_url, status_effect, status_effect_chance')
        .in('id', creatureIds)
  ...
  if (isFirstUniqueScan && ['oggetto', 'uovo', 'indizio'].includes(qr.type)) {
    ...
    evtAdmin.from('player_game_events').insert({
      user_id: user.id, session_id: sessionId, type: 'qr_redeemed', payload: evtPayload as Json,
    }).then(undefined, () => {})
  }
