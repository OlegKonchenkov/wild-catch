...
// POST /api/admin/items/[id]/artwork
...
export async function POST(
  ...
  const enhancedPrompt = `Game equipment item icon for a mobile creature-catching RPG set in the Italian Adriatic coast and Apennine forests. Single piece of gear, painterly fantasy style, vibrant colors, centered, no text, square format, clean transparent-ish background suitable for an inventory icon. The item: ${prompt.trim()}`
  ...
  try {
    ...
    const storagePath = `items/${id}.png`
  ...
  const { data, error: dbError } = await admin.from('items')
