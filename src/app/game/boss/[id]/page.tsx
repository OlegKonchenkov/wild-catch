    ...
    <div className="flex flex-col items-center justify-center h-full px-6 gap-6">
      ...
      {won && reward && (
        <div className="w-full rounded-2xl p-4 space-y-3"
          style={{ background: 'rgba(247,200,65,0.06)', border: '1px solid rgba(247,200,65,0.2)' }}>
          <p className="text-xs text-white/40 uppercase tracking-wider font-semibold text-center">Ricompense</p>
          ...
          {reward.creature && (
            <div className="rounded-xl overflow-hidden"
              ...
              <p className="text-xs text-white/40 px-3 pt-2.5 pb-1 font-semibold uppercase tracking-wider">Creatura catturata!</p>
                ...
                <div>
                  ...
                  <p className="text-white/40 text-xs mt-0.5">
                    {RARITY_COLORS[reward.creature.rarity as keyof typeof RARITY_COLORS]
                      ? <span style={{ color: RARITY_COLORS[reward.creature.rarity as keyof typeof RARITY_COLORS] }}>
                          {RARITY_LABELS[reward.creature.rarity as keyof typeof RARITY_LABELS] ?? reward.creature.rarity}
                        </span>
                      : reward.creature.rarity
                    }
                    {reward.creature.element && ` · ${ELEMENT_EMOJI[reward.creature.element as keyof typeof ELEMENT_EMOJI] ?? ''} ${reward.creature.element}`}
                  </p>
                </div>
              </div>
            </div>
